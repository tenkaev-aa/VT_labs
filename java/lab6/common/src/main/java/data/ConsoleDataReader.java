...
import input_object.FieldMetaReader;
...
public class ConsoleDataReader<T> implements DataReader<T> {
  ...
  public Optional<Float> readFloatWithMeta(BiConsumer<T, Float> setter) throws IOException {
    ...
    return readFloat(prompt, errorMessage, positive, nullable, (float) min, (float) max);
  }
  ...
  public Optional<Double> readDouble(
      ...
      throws IOException {
    return readNumber(
        prompt, errorMessage, mustBePositive, allowNull, min, max, Double::parseDouble);
  }
    ...
    while (true) {
      ...
      try {
        ...
        if (min != null && value.doubleValue() < min.doubleValue()) {
          System.out.println("Ошибка: " + errorMessage + " (минимальное значение: " + min + ")");
          continue;
        }
        if (max != null && value.doubleValue() > max.doubleValue()) {
          System.out.println("Ошибка: " + errorMessage + " (максимальное значение: " + max + ")");
          continue;
        }
