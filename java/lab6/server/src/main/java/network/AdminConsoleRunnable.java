...
/**
 * Потоковая админ-консоль для управления сервером.
 * Поддерживает команды: switch to admin, status, save_internal, exit, shutdown.
 */
public class AdminConsoleRunnable implements Runnable {
    ...
    public AdminConsoleRunnable(Scanner scanner, CityManager cityManager, SaveCommand saveCommand, String adminPassword) {
    ...
    public void run() {
        while (true) {
            ...
            if (adminMode) {
                switch (input) {
                    case "status" -> ServerLogger.log("[SERVER] Элементов в коллекции: " +
                            cityManager.getCollectionSize());
                ...
                System.out.println("Неизвестная команда. Введите 'switch to admin' для доступа к функциям администратора.");
            }
        }
    }
}
